    ...
    unsigned dagCreateDevice)
{
    try
    {
		...
		compileKernel(epoch, dagSize/256);
            ...
            if (!hostDAG)
            {
                if((m_device_num == dagCreateDevice) || !_cpyToHost){ //if !cpyToHost -> All devices shall generate their DAG
                    ...
                    ethash_generate_dag(dagSize, s_gridSize, s_blockSize, m_streams[0]);
  					hash64_t d[8];
		            CUDA_SAFE_CALL(cudaMemcpy(reinterpret_cast<void*>(d), dag, 4*sizeof(hash64_t), cudaMemcpyDeviceToHost));
					//cudalog << context.epoch_number;
					//cudalog << to_hex(*(hash64_t*)context.light_cache);
		            CUDA_SAFE_CALL(cudaMemcpy(reinterpret_cast<void*>(d+4), dag+dagNumItems-2, 4*sizeof(hash64_t), cudaMemcpyDeviceToHost));
		            for(int k=0;k<2;k++)for (int i=0;i<4;i++) cudalog <<k<<" "<< to_hex(d[4*k+i]);
...
void CUDAMiner::run_ethash_search(
	uint32_t gridSize,
	uint32_t blockSize,
	cudaStream_t stream,
    ...
    if (!TEST_PROGPOW) {
    cu_ethash_search(gridSize, blockSize, stream, g_output, start_nonce, parallelHash);
    } else {
        uint8_t* h;
		...
		void *args[] = {&start_nonce, &h, &m_current_target, &m_dag, &g_output};
        ...
        for (current_index = 0; current_index < s_numStreams; current_index++, current_nonce += batch_size) {
            ...
            if (found_count) {
                ...
                for (unsigned int j = 0; j < found_count; j++) {
                    ...
                    //if (s_noeval)
                        memcpy(mixes[j].data(), (void *)(buffer->result[j].mix), 32);                    
                }
                ...
                for (uint32_t i = 0; i < found_count; i++)
                    ...
                    else
                    {
                        ...
                        cwarn<<i<<" "<<w.epoch<<" "<<toHex(nonces[i]);
                        cwarn<<"header "<<(w.header);
                        cwarn<<" r.value "<<(r.value);
                        cwarn<<"boundary "<<(w.boundary);
                        cwarn<<"cuda mixes "<<(mixes[i]);
                        cwarn<<"cpu mixes: "<<(r.mixHash);
